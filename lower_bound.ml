module Make(F : Structures.Field) = struct
    ...
    let cut_to_edges : FGraph.t -> IntSet.t -> (int * int) list =
        fun grph cut ->
            ...
            (elements (filter (fun j -> not (IntSet.mem j cut)) cut
            )))) cut []
    ...
    let solve_w_constr : FGraph.t -> IntSetSet.t -> (FMatrix.t * F.t) =
         fun grph constr ->
             ...
             let constr_b = make (nb_l constr_mat) 1 F.(e_add +. e_add) in
                 debug "constr_mat" constr_mat;
                 debug "constr_b" constr_b;
             let degree_mat = degrees_to_mat grph corr in
             let degree_b = make (nb_l degree_mat) 1 F.(e_add +. e_add) in
                 debug "degree_mat" degree_mat;
                 debug "degree_b" degree_b;
             ...
             and constr_les = (ones 0 0, ones 0 0)
    ...
    let naive_solve : FGraph.t -> (FMatrix.t * F.t) =
        fun grph ->
            let constr = subsets (FGraph.vertices grph) in
